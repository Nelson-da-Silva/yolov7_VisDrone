...
# python distortion_scripts/apply-upscaled-fisheye.py --input_dir=VisDrone/VisDrone2019-DET-test-dev -f=<choose an input focal length>
#   - where the input directory has two subdirectories: images and labels where the label
...
#   the labels are distorted in the same way
def distort_labels(label_file, f, u_x, u_y, crop=False, top_border=0, left_border=0):
    ...
    result_file = open(str(parent_dir) + "/upscaled-fisheye/" + args.focal_length + "/labels/" + label_file.stem+".txt", "w")
...
def main():
    ...
    if args.input_dir:
        ...
        result_images_dir = Path(args.input_dir+"/upscaled-fisheye/" + args.focal_length + "/images/")
        result_labels_dir = Path(args.input_dir+"/upscaled-fisheye/" + args.focal_length + "/labels/")
